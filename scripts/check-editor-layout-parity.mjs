...
export async function validateEditorLayoutParity(root) {
  ...
  const cssContract = [
    ...
    [/--g7pb-page-content-max:\s*80rem;[\s\S]*\.g7pb-full-site-page--template\s*\{[^}]*(?:width|max-width):[^;}]*var\(--g7pb-page-content-max\)/,
      '편집기 template shell은 공통 1280px 콘텐츠 폭 토큰을 사용해야 합니다.'],
    [/--g7pb-page-gutter:\s*1\.25rem;/,
      '편집기와 공개 shell이 공유하는 반응형 여백 토큰이 필요합니다.'],
