...
가운데 캔버스는 결과를 보는 미리보기가 아니라 선택 요소 중심의 구조화된 WYSIWYG입니다. 29개 내장 블록은 같은 선택·편집 규칙을 사용하며, 우측 Inspector는 고급 구조와 데이터 설정을 보완합니다.
...
5. 블록 글자 크기·정렬·배경·세로 여백은 제한된 디자인 토큰으로 즉시 반영합니다.
...
- 글자 크기·정렬·배경·여백은 compiler와 공개 CSS가 이미 지원하는 typed token만 사용합니다.
...
- 이미지: Hero, Hero Split, Hero Slider, Logo Cloud, Logo Carousel, Team, Gallery, Testimonials, Testimonial Slider, Article List
- 경로: Hero 계열, CTA, Contact, Logo 계열, Pricing, Team, Process, Article List, Event Schedule, Download Resources, Map Directions
- 반복 구조: Features, Slider, Logo, Stats, Pricing, Team, Gallery, Chart, Testimonials, FAQ, Process, Tabs, Comparison, Article, Event, Download
...
- 디자인 토큰은 별도 소형 패널에 묶고 현재 선택값을 `aria-pressed`로 표시합니다.
- 반복 항목 삭제가 최소 개수에 걸리면 버튼을 비활성화하고 이유를 접근 가능한 label로 제공합니다.

## 회귀 게이트

- Unit: 29개 계약의 유일성, route/media path 해석, 중첩 값 불변 갱신, 최소·최대 메타데이터
- Component: 실제 캔버스 선택, 이미지·route dialog, 토큰 변경, 반복 항목 복제 후 canonical document 반영
