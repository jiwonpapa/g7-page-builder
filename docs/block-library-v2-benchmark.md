...
상태: **기존 프리셋·페이지 킷 확장 조사의 보존 기록**. 아래 경쟁사 수량과 12개 블록·52개 레이아웃·95개 preset은 당시 비교와 수용 기준입니다. 현재 경쟁사 기능 보장이나 복합 블록 내부 편집 완료를 뜻하지 않습니다.

현재 삽입 분류·필드·슬롯 경계는 [편집 정책](productization/editing-policy.md), 실행 순서와 증거는 [개발 계획](productization/editor-plan.md)과 [진척 표시판](productization/editor-progress.md)을 따릅니다. 기술적 preset과 완성 섹션은 같은 분류가 아니며, 제목·버튼 preset은 해당 요소의 변형입니다. 아래 ‘완성 섹션’이라는 기존 UI 용어와 프리셋 증가를 새 편집 능력의 진척으로 합산하지 않습니다. 원래 레이아웃 목록·호환 계약·수용 기준은 과거 기록으로 유지합니다.
