...
export async function validateEditorAcceptanceContract(root) {
  ...
  const requiredEvidence = [
    ...
    [/field\.hover\(\{\s*position:\s*pointer\.start\s*\}\)/, 'iframe 축척을 반영한 실제 pointer 시작 이동이 필요합니다.'],
    [/field\.hover\(\{\s*position:\s*pointer\.end\s*\}\)/, 'iframe 축척을 반영한 실제 pointer 범위 드래그가 필요합니다.'],
    [/field\.click\(\{\s*position:\s*pointer\.start\s*\}\)/, '범위 선택 전 contenteditable을 실제 pointer click으로 활성화해야 합니다.'],
    [/expect\(field\)\.toBeFocused\(\)/, '실제 pointer click 뒤 contenteditable focus를 확인해야 합니다.'],
    [/page\.mouse\.up\s*\(/, '실제 pointer 선택을 위한 page.mouse.up이 필요합니다.'],
    [/field\.click\(\{\s*position:\s*pointer\.end\s*\}\)/, '선택 해제도 iframe 축척을 반영한 실제 pointer click으로 검증해야 합니다.'],
