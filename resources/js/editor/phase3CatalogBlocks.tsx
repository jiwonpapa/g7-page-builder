...
function LogoCarouselPreview(props: LogoCarouselEditorProps & { id: string }): React.ReactElement {
  return <Frame id={props.id} type="logo-carousel" motion={props.motion} elementStyles={props.elementStyles}><div className={`g7pb-preview-logo-carousel ${surfaceClass(props)}`}><header><small data-g7pb-inline-field="eyebrow">{props.eyebrow}</small><RichTextCanvasField as="h2" className="g7pb-preview-richtext" fieldPath="heading">{props.heading}</RichTextCanvasField></header><div>{normalizeLogos(props.logos).map((logo, index) => <span key={`${logo.name}-${index}`}>{logo.imageSrc ? <img data-g7pb-media-field={`logos.${index}.imageSrc`} src={logo.imageSrc} alt={logo.imageAlt} /> : <b data-g7pb-inline-field={`logos.${index}.name`}>{inlineArrayContent(props.logos, index, 'name', logo.name)}</b>}</span>)}</div><footer><button type="button" aria-label="이전 로고">←</button><i>자동 {props.autoplay === 'yes' ? '재생' : '정지'}</i><button type="button" aria-label="다음 로고">→</button></footer></div></Frame>;
}
