...
export function canonicalProductionBlockToPuck(block: PageBuilderBlock): { type: ProductionComponentType; props: ProductionCatalogEditorComponents[ProductionComponentType] } | null {
  ...
  if (block.type === CARD_GRID_BLOCK_TYPE) return { type: 'CardGrid', props: { eyebrow: asString(props.eyebrow), heading: asString(props.heading), items: normalizeCards(props.items), columns: props.columns === 2 || props.columns === '2' ? '2' : '3', variant: props.variant === 'plain' ? 'plain' : 'outlined', layout: props.layout === 'grid' || props.layout === 'rail' || props.layout === 'editorial' || props.layout === 'numbered' ? props.layout : 'bento', ...common(block, { surface: 'default', spacing: 'normal' }) } };
