...
final class SitePartFixtureState
{
    ...
    private function rows(string $locale): array
    {
        ...
        if ($active > 1) {
            throw new RuntimeException('Multiple active sets; no restoration attempted.');
        }
        ksort($result);
    ...
    {
        if ($expected !== $actual) {
            throw new RuntimeException('Site Part pointer/audit CAS mismatch; journal retained.');
        }
    }

    public function begin(string $locale): array
    {
        if (! preg_match('/^[a-z]{2}(?:[-_][A-Za-z]{2})?$/D', $locale)) {
            throw new InvalidArgumentException('Invalid fixture locale.');
        }
    ...
    public function command(array &$state, string $action, array $input): void
    {
        ...
        if ($action === 'checkpoint') {
            ...
            if (! is_string($target) || ! isset($state['owned'][$target])) {
                throw new RuntimeException('No owned activation intent.');
            }
            $before = $state['expected'];
            if (array_keys($before) !== array_keys($rows) || ! $rows[$target]['is_active']) {
                throw new RuntimeException('Activation did not select the owned set.');
            }
        ...
        if ($action === 'register') {
            $id = $input['id'] ?? '';
            if (! preg_match('/^[a-f0-9-]{36}$/D', $id) || isset($state['expected'][$id]) || ! isset($rows[$id])) {
                throw new RuntimeException('API-created UUID is not a new set.');
            }
            if (! is_string($state['pending_create']) || $rows[$id]['title'] !== $state['pending_create']) {
                throw new RuntimeException('API-created UUID does not match the reserved fixture title.');
            }
            $new = $rows[$id];
            unset($rows[$id]);
            ...
            if ($new['is_active'] !== ! $hadActive) {
                throw new RuntimeException('Unexpected create activation state.');
            }
            $state['owned'][$id] = $new;
            $rows[$id] = $new;
            ksort($rows);
        ...
        $this->unchanged($state['expected'], $rows);
        if ($action === 'reserve') {
            if ($state['pending_create'] !== null || $state['activation'] !== null) {
                throw new RuntimeException('Unfinished fixture operation.');
            }
            $title = $input['title'] ?? '';
            if (! preg_match('/^E2E-owned-[a-f0-9-]{36}-[0-9]+$/D', $title)) {
                throw new RuntimeException('Invalid reserved title.');
            }
            $state['pending_create'] = $title;
        } elseif ($action === 'prepare') {
            $id = $input['id'] ?? '';
            if (! isset($state['owned'][$id]) || $state['pending_create'] !== null || $state['activation'] !== null) {
                throw new RuntimeException('Activation requires an idle owned set.');
            }
            $state['activation'] = $id;
            ...
            foreach ($state['baseline'] as $id => $row) {
                if ($row['is_active']) {
                    ($this->write)($id, ['is_active' => true, 'updated_by' => $row['updated_by'], 'updated_at' => $row['updated_at']]);
                }
            }
...
function g7pbSitePartFixtureCommand(): void
{
    ...
    if (! $stream || ! flock($stream, LOCK_EX)) {
        throw new RuntimeException('Cannot lock fixture journal.');
    }
    try {
        ...
        if ($action !== 'restore-all' && ! preg_match('/^[a-f0-9-]{36}$/D', $session)) {
            throw new RuntimeException('Invalid fixture session.');
        }
        $updated = $journal;
        try {
            DB::transaction(function () use (&$updated, $session, $action, $payload): void {
                $state = new SitePartFixtureState(
                    fn ($locale) => DB::table('g7pb_site_part_sets')->where('locale', $locale)->orderBy('id')->lockForUpdate()
                        ->get(['id', 'title', 'locale', 'is_active', 'updated_by', 'updated_at'])->all(),
                    fn ($id, $values) => DB::table('g7pb_site_part_sets')->where('id', $id)->update($values),
                );
                if ($action === 'begin') {
                    foreach ($updated['sessions'] as $item) {
                        if (! $item['restored']) {
                            throw new RuntimeException('Another fixture session is still active.');
                        }
                    }
                    if (isset($updated['sessions'][$session])) {
                        throw new RuntimeException('Fixture session already exists.');
                    }
                    $updated['sessions'][$session] = $state->begin($payload['locale'] ?? '');
                } elseif ($action === 'restore-all') {
                    foreach ($updated['sessions'] as &$item) {
                        $state->command($item, 'restore', []);
                    }
                    $updated['closed'] = true;
                } else {
                    if (! isset($updated['sessions'][$session])) {
                        throw new RuntimeException('Unknown fixture session.');
                    }
            ...
            if (! $output) {
                throw new RuntimeException('Cannot stage fixture journal; previous journal retained.');
            }
            try {
                if (fwrite($output, $encoded) !== strlen($encoded) || ! fflush($output) || ! fsync($output)) {
                    throw new RuntimeException('Cannot persist fixture journal; previous journal retained.');
                }
            } finally {
                fclose($output);
            }
            if (! rename($staged, $path)) {
                throw new RuntimeException('Cannot replace fixture journal; previous journal retained.');
            }
        }
    } finally {
