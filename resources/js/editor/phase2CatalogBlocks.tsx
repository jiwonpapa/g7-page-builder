...
export const phase2CatalogComponentConfigs: Config<Phase2CatalogEditorComponents>['components'] = {
  Testimonials: {
    ...
    fields: { eyebrow: { type: 'text', label: '보조 문구', contentEditable: true }, heading: { type: 'text', label: '제목', contentEditable: true }, layout: { type: 'radio', label: '배치', options: [{ label: '그리드', value: 'grid' }, { label: '집중형', value: 'spotlight' }] }, items: { type: 'array', label: '후기', min: 2, max: 8, defaultItemProps: (index) => ({ quote: '고객 경험을 입력하세요.', name: `고객 ${index + 1}`, role: '담당자', company: '회사명', avatarSrc: '', avatarAlt: '', rating: 5 }), getItemSummary: (item) => `${item.name} · ${item.company}`, arrayFields: { quote: { type: 'textarea', label: '후기', contentEditable: true }, name: { type: 'text', label: '이름', contentEditable: true }, role: { type: 'text', label: '역할', contentEditable: true }, company: { type: 'text', label: '회사', contentEditable: true }, avatarSrc: createMediaField('프로필 이미지'), avatarAlt: { type: 'text', label: '대체 텍스트' }, rating: { type: 'select', label: '평점', options: [1, 2, 3, 4, 5].map((value) => ({ label: `${value}점`, value })) } } }, ...appearanceFields, motion: createMotionField(['none', 'reveal', 'stagger']) },
    render: (props) => <TestimonialsPreview {...props} />,
  },
  FaqAccordion: {
    label: 'FAQ 아코디언', defaultProps: DEFAULT_FAQ,
    fields: { eyebrow: { type: 'text', label: '보조 문구', contentEditable: true }, heading: { type: 'text', label: '제목', contentEditable: true }, behavior: { type: 'radio', label: '열림 방식', options: [{ label: '하나만', value: 'single' }, { label: '여러 개', value: 'multiple' }] }, openFirst: { type: 'radio', label: '첫 항목', options: [{ label: '열기', value: true }, { label: '닫기', value: false }] }, items: { type: 'array', label: '질문과 답변', min: 2, max: 12, defaultItemProps: (index) => ({ question: `질문 ${index + 1}`, answer: '답변을 입력하세요.' }), getItemSummary: (item) => item.question, arrayFields: { question: { type: 'text', label: '질문', contentEditable: true }, answer: { type: 'textarea', label: '답변', contentEditable: true } } }, ...appearanceFields, motion: createMotionField(['none', 'reveal']) },
