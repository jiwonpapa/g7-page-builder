...
5. 문장 일부를 드래그하면 Tiptap 인라인 도구로 그 범위에만 글꼴·크기·굵기·밑줄·색상·링크를 적용합니다. 클릭만 한 요소 전체의 공통 글꼴·정렬은 작은 요소 벌룬에서 조정합니다.
...
- 텍스트·버튼을 클릭하면 요소 경계 위에 작은 요소 전체 벌룬을 표시하고, 바깥 영역 클릭이나 `Escape`에서 닫습니다.
...
- Component: 실제 캔버스 선택, 범위 편집과 요소 벌룬 분리·닫힘, route/media overlay 유지·적용, 선택 요소 token 변경, 형제 요소 비전파, 즉시 dirty 전환과 이탈 방지, 반복 항목 복제 후 canonical document 반영
- Browser: 생성 → 직접 편집 → 범위 서식 → 이탈 방지 → 이미지/경로 → 반복 구조 → 미리보기 → 발행 흐름
