...
- 일반 구현 차수는 `PROFILE=scoped`로 시작하고 `make task-integrate-scoped`로 통합한다. Python 제어부가 실제 변경과 관련 테스트를 선택한다. 명령·대상 파일·의존 입력·도구 환경이 같은 성공 결과만 재사용하며, 전체 Git tree나 SHA만으로 재사용을 판정하지 않는다. 버전 숫자만 변경했다고 full을 실행하지 않는다.
- 범위 검증 실패 시 실패 gate와 그 downstream만 다시 실행한다. 같은 명령·대상·의존 입력의 성공 gate를 강제로 반복하거나, 작은 수정 뒤 전체 coverage·전체 프런트·전체 E2E를 다시 실행하지 않는다. 입력을 확정할 수 없는 동적 검사는 성공 재사용만 금지하며 범위를 확대하지 않는다. 관련 테스트를 찾지 못하면 해당 task에서 명시적 관련 테스트를 추가한다.
- 제품 소스 변경 없이 브라우저 테스트 정의·등록만 고친 하네스 작업은 선택 spec의 수집 검사와 관련 하네스 테스트로 확인하며 제품 브라우저 실행 성공으로 보고하지 않는다. 실제 제품 소스와 시나리오가 함께 바뀌면 해당 spec을 runtime lease 아래 실행한다. 과거 셸 전체 호환 fixture는 명시적 `quality-coordination-compat`에서만 실행한다.
...
- 페이지 빌더는 자기 모듈의 문서·리비전·발행본과 페이지 주소 할당 테이블만 소유한다. 추가 주소는 기존 G7 route와 충돌하면 활성화하지 않으며 기본 `/pages/:slug`와 기존 G7 페이지·메뉴 원본을 보존한다.
