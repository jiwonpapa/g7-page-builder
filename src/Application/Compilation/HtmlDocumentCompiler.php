...
final class HtmlDocumentCompiler implements DocumentCompilerPort
{
    ...
    private function compileNotice(array $props): string
    {
        ...
        return '<section class="g7pb-block g7pb-content-notice g7pb-content-notice--'.$tone.' '.$appearance.'" data-testid="page-builder-rendered-block" data-block-type="notice" role="'.$role.'"><span class="g7pb-content-notice__icon" aria-hidden="true"></span><div><h2 class="g7pb-content-notice__title">'.$this->escape($title).'</h2><div class="g7pb-content-notice__body">'.$this->sanitizeRichText($body).'</div></div>'.$action.'</section>';
    }
    ...
    private function compileCardGrid(array $props): string
    {
        ...
        foreach (array_values($items) as $index => $item) {
            ...
            $compiled[] = '<article class="g7pb-card-grid__item">'.($kicker === '' ? '' : '<p class="g7pb-card-grid__kicker">'.$this->escape($kicker).'</p>').'<h3>'.$this->escape($title).'</h3>'.($body === '' ? '' : '<div class="g7pb-card-grid__body">'.$this->sanitizeRichText($body).'</div>').$link.'</article>';
        }
    ...
    private function compileHeroSplit(array $props): string
    {
        ...
        if ($body !== null && $body !== '') {
            $copy[] = '<div class="g7pb-hero-split__body">'.$this->sanitizeRichText($body).'</div>';
        }
    ...
    private function compileHeroSlider(array $props): string
    {
        ...
        foreach (array_values($slides) as $index => $slide) {
            ...
            if ($body !== null && $body !== '') {
                $copy .= '<div class="g7pb-hero-slider__body">'.$this->sanitizeRichText($body).'</div>';
            }
    ...
    private function compileProcessTimeline(array $props): string
    {
        ...
        foreach (array_values($items) as $index => $item) {
            ...
            $compiled[] = '<li><span class="g7pb-process__number">'.str_pad((string) ($index + 1), 2, '0', STR_PAD_LEFT).'</span><h3>'.$this->escape($title).'</h3><div class="g7pb-process__body">'.$this->sanitizeRichText($body).'</div>'.$link.'</li>';
        }
    ...
    private function compileTabs(array $props): string
    {
        ...
        foreach (array_values($items) as $index => $item) {
            ...
            $panels[] = '<article role="tabpanel" data-g7pb-tab-panel="'.$index.'" tabindex="0"'.($selected ? '' : ' hidden').'><h3>'.$this->escape($itemHeading).'</h3><div class="g7pb-tabs__body">'.$this->sanitizeRichText($body).'</div></article>';
        }
    ...
    private function compileArticleList(array $props): string
    {
        ...
        foreach (array_values($items) as $index => $item) {
            ...
            $compiled[] = '<article><figure>'.$media.'</figure><div>'.($meta === [] ? '' : '<p class="g7pb-articles__meta">'.implode('<i>·</i>', $meta).'</p>').'<h3><a href="'.$this->escapeAttribute($url).'">'.$this->escape($title).'</a></h3><div class="g7pb-articles__summary">'.$this->sanitizeRichText($summary).'</div><a class="g7pb-articles__link" href="'.$this->escapeAttribute($url).'">읽어보기 <span aria-hidden="true">→</span></a></div></article>';
        }
    ...
    private function compileTestimonialSlider(array $props): string
    {
        ...
        foreach (array_values($items) as $index => $item) {
            ...
            $meta = ($role === '' ? '' : '<span class="g7pb-testimonial-role">'.$this->escape($role).'</span>')
                .($role !== '' && $company !== '' ? '<i aria-hidden="true"> · </i>' : '')
                .($company === '' ? '' : '<span class="g7pb-testimonial-company">'.$this->escape($company).'</span>');
            $slides[] = '<blockquote class="g7pb-hero-slider__slide g7pb-testimonial-slider__slide" role="group" aria-roledescription="slide" aria-label="'.($index + 1).' / '.count($items).'"><p class="g7pb-testimonial-slider__rating" aria-label="5점 만점에 '.$rating.'점">'.str_repeat('★', $rating).'</p><div class="g7pb-testimonial-slider__quote">'.$this->sanitizeRichText($quote).'</div><footer><figure>'.$avatar.'</figure><cite><strong>'.$this->escape($name).'</strong>'.$meta.'</cite></footer></blockquote>';
        }
    ...
    private function compileEventSchedule(array $props): string
    {
        ...
        foreach (array_values($items) as $index => $item) {
            ...
            $compiled[] = '<li><time datetime="'.$this->escapeAttribute($date.($time === '' ? '' : 'T'.$time)).'"><strong>'.$this->escape($date).'</strong>'.($time === '' ? '' : '<span>'.$this->escape($time).'</span>').'</time><article>'.($location === '' ? '' : '<p class="g7pb-events__location">'.$this->escape($location).'</p>').'<h3>'.$this->escape($title).'</h3><div class="g7pb-events__description">'.$this->sanitizeRichText($description).'</div>'.$action.'</article></li>';
        }
    ...
    private function compileDownloadResources(array $props): string
    {
        ...
        foreach (array_values($items) as $index => $item) {
            ...
            $compiled[] = '<li><span class="g7pb-downloads__type">'.$this->escape(mb_strtoupper($fileType)).'</span><div><h3>'.$this->escape($title).'</h3>'.($description === '' ? '' : '<div class="g7pb-downloads__description">'.$this->sanitizeRichText($description).'</div>').'<small>'.$fileMeta.'</small></div><a href="'.$this->escapeAttribute($url).'" download>'.$this->escape($buttonLabel).' <span aria-hidden="true">↓</span></a></li>';
        }
    ...
    private function elementAppearanceXPath(string $type, string $fieldPath): ?string
    {
        ...
        return match ($type) {
            ...
            self::HERO_SLIDER_TYPE => $collection === 'slides' ? match ($leaf) {
                ...
                'body' => '(.//*['.$hasClass('g7pb-hero-slider__slide').'])['.$index.']//*['.$hasClass('g7pb-hero-slider__body').']',
            ...
            self::PROCESS_TIMELINE_TYPE => $collection === 'items' ? match ($leaf) {
                ...
                'body' => '(.//ol/li)['.$index.']/*['.$hasClass('g7pb-process__body').']',
            ...
            self::TABS_TYPE => $collection === 'items' ? match ($leaf) {
                ...
                'body' => '(.//*['.$hasClass('g7pb-tabs__panels').']/article)['.$index.']/*['.$hasClass('g7pb-tabs__body').']',
                default => null,
            } : null,
            ...
            self::ARTICLE_LIST_TYPE => $collection === 'items' ? match ($leaf) {
                ...
                'summary' => '(.//*['.$hasClass('g7pb-articles__items').']/article)['.$index.']//*['.$hasClass('g7pb-articles__summary').']',
                default => null,
            } : null,
            self::EVENT_SCHEDULE_TYPE => $collection === 'items' ? match ($leaf) {
                ...
                'title' => '(.//ol/li)['.$index.']//h3',
                'description' => '(.//ol/li)['.$index.']//*['.$hasClass('g7pb-events__description').']',
                'buttonLabel' => '(.//ol/li)['.$index.']//article/a',
                default => null,
            } : null,
            self::DOWNLOAD_RESOURCES_TYPE => $collection === 'items' ? match ($leaf) {
                'title' => '(.//ul/li)['.$index.']//h3',
                'description' => '(.//ul/li)['.$index.']//*['.$hasClass('g7pb-downloads__description').']',
