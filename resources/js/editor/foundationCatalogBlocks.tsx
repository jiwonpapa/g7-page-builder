...
export function canvasTextValue(value: unknown): string {
  ...
  if ('value' in props) return canvasTextValue(props.value);
  if ('content' in props) return canvasTextValue(props.content);
  return canvasTextValue(props.children);
}
...
function HeadingPreview(props: HeadingEditorProps & { id: string }): React.ReactElement {
  ...
  return <Frame id={props.id} type="heading" motion={props.motion} elementStyles={props.elementStyles}>
    <div className={`g7pb-preview-heading ${surfaceClass(props)}`}>
      {canvasTextValue(props.eyebrow) ? <small data-g7pb-inline-field="eyebrow">{props.eyebrow}</small> : null}
