...
const TONE_VALUES = ['default', 'muted', 'accent', 'contrast'] as const;

type FontValue = typeof FONT_VALUES[number];
type SizeValue = typeof SIZE_VALUES[number];
type ToneValue = typeof TONE_VALUES[number];

function enumAttribute<T extends string>(key: 'font' | 'size' | 'tone', values: readonly T[], fallback: T) {
  return {
    default: fallback,
...
export const G7TextStyleMark = Mark.create({
  ...
  parseHTML() {
    return [{ tag: 'span[data-g7pb-font], span[data-g7pb-size], span[data-g7pb-tone]' }];
  },
...
function selectedMark(editor: Editor | null): { font: FontValue; size: SizeValue; tone: ToneValue } {
...
function G7RichTextInlineMenu({
  ...
  const updateMark = (patch: Partial<{ font: FontValue; size: SizeValue; tone: ToneValue }>): void => {
    if (!editor || readOnly) return;
    const next = { ...selectedMark(editor), ...patch };
    if (next.font === 'inherit' && next.size === 'base' && next.tone === 'default') {
