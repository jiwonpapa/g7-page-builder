...
3. 37종 block schema·editor·PHP compiler·public renderer, 좌측 미리보기 DnD와 반응형 iframe 기기 전환, 핵심 lifecycle PC·태블릿·모바일 E2E
4. 별도 문서함, 메타수정, 최근 리비전 조회·미리보기·새 초안 복원·재발행 rollback·공개 해제
5. 5종 typed motion preset, 공개 런타임, reduced-motion·CSP 회귀시험
6. 독립 Block Pack 계약, 37개 내장 블록 Registry, 카탈로그 검색·분류·즐겨찾기
7. G7 공개 API 기반 게시글·상품 목록·상세, pagination, 공통 전체·비회원·회원 표시 조건
8. MediaPort 이미지·다운로드 파일 업로드와 기존 자산 선택
9. Data/Code Pack ZIP 설치·상태 전환·사용량 기반 제거, GitHub Release digest 업데이트, Ed25519 Code Pack runtime
10. Page Builder Site Part 2단 메뉴·PC 드롭다운·모바일 하위 메뉴·Header/Footer 프리셋·전체 라우트 선택기

다음:

1. 이벤트 캘린더 공개 데이터 capability와 100개 블록 성능 기준
2. 결정적 시각 baseline 확대와 nested slot 요구 검증

G7 데이터 블록은 공개 API capability가 있을 때만 활성화하며 `sirsoft-board`·`sirsoft-ecommerce`를 hard dependency로 만들지 않습니다.
