...
- 메뉴: 할당한 주소를 복사하여 PB의 헤더·푸터 편집에서 메뉴 링크로 연결하고 세트를 발행한다. 기본 템플릿 메뉴는 해당 템플릿의 편집 동선을 사용한다. `/admin/menus`는 관리자 사이드바 관리이므로 방문자 메뉴 연결에 사용하지 않는다. PB는 G7 메뉴 테이블을 직접 수정하지 않는다.
