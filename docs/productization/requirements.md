...
상태: **기존 8차 계획의 요구사항 ID·배치·시험 연결 기록**. 현행 요구와 완료 조건은 [편집 정책](editing-policy.md)과 [편집 기능 개발 계획](editor-plan.md), 실제 상태는 [진척 원장](editor-progress.json)과 [표시판](editor-progress.md)을 따른다. 아래 예정 배치와 당시 ‘남은’ 상태를 현재 구현 상태로 복사하지 않는다. 기존 요구사항·실행 증거를 보존하되 새 계획의 기능은 별도 ID와 실제 근거로 추적한다.
