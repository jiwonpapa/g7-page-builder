...
유명 빌더의 화면이나 유료 소스 코드를 복제하지 않고 반복되는 정보 구조를 typed block으로 정규화합니다. 현재 카탈로그는 정적·상호작용 콘텐츠 37종, G7 공개 데이터 6종, 문의 폼 1종, 찾아오기 1종으로 총 45종입니다. 9개 용도 분류, 6개 Quick Add, 140개 실제 렌더 썸네일, 모든 타입을 덮는 95개 한국어 프리셋을 제공합니다. 다만 이 수량을 완성도로 보지 않으며, 아래 P000·P0 항목과 45종 전수 하네스가 통과할 때까지 신규 block type 추가를 중단합니다.
