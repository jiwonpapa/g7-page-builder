...
export async function validateEditorLayoutParity(root) {
  ...
  const requiredEvidence = [
    ...
    [/waitForResponse\(\(candidate\)\s*=>\s*new URL\(candidate\.url\(\)\)\.pathname\.startsWith\(['"]\/api\/layouts\/preview\/['"]\)/,
      'template shell은 실제 G7 preview layout API 응답 뒤에 geometry를 측정해야 합니다.'],
