...
export async function validateEditorLayoutParity(root) {
  ...
  const cssContract = [
    ...
    [/\[data-g7pb-heading-level\]\s+:where\(\*\)\s*\{[^}]*margin:\s*0\s*!important;[^}]*font-family:\s*inherit\s*!important;[^}]*font-size:\s*inherit\s*!important;[^}]*font-weight:\s*inherit\s*!important;[^}]*line-height:\s*inherit\s*!important;[^}]*overflow-wrap:\s*inherit\s*!important;[^}]*white-space:\s*inherit\s*!important;[^}]*word-break:\s*inherit\s*!important;/,
      'Puck의 실제 제목 leaf는 wrapper의 WYSIWYG typography와 줄바꿈 규칙을 상속해야 합니다.'],
