...
- 상품 스크린샷은 공식 Page Kit을 G7 활성 User Template에 실제 적용·발행한 뒤 PC 1425px, 태블릿 805px, 모바일 375px에서 캡처한 전체 화면을 WebP로 보존한다.
- 각 상품은 컴파일된 정적 데모와 3개 실제 화면을 제공한다. 데모 요청에서는 문서를 다시 컴파일하지 않는다.

## Publish readiness

- Hero 이미지는 즉시 사용할 수 있지만 팀·연사·기사 보조 이미지는 업종과 운영 주체에 맞게 교체하거나 의도적인 텍스트형으로 유지한다.
- `/` 샘플 링크와 자료·일정·문의 동작은 적용 후 실제 경로와 운영 정보로 교체한다.
- Page Kit 적용 확인창에서 위 항목을 발행 전 체크리스트로 다시 안내한다.
