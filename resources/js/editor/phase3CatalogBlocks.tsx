...
function TestimonialSliderPreview(props: TestimonialSliderEditorProps & { id: string }): React.ReactElement {
  ...
  return <Frame id={props.id} type="testimonial-slider" motion={props.motion}><div className={`g7pb-preview-testimonial-slider ${surfaceClass(props)}`}><header><small data-g7pb-inline-field="eyebrow">{props.eyebrow}</small><h2 data-g7pb-inline-field="heading">{props.heading}</h2></header><blockquote><span>{'★'.repeat(item.rating)}</span><p data-g7pb-inline-field={`items.${active}.quote`}>“{item.quote}”</p><footer>{item.avatarSrc ? <img data-g7pb-media-field={`items.${active}.avatarSrc`} src={item.avatarSrc} alt={item.avatarAlt} /> : <i data-g7pb-media-field={`items.${active}.avatarSrc`} aria-hidden="true">{item.name.slice(0, 1)}</i>}<strong data-g7pb-inline-field={`items.${active}.name`}>{item.name}</strong><small><span data-g7pb-inline-field={`items.${active}.role`}>{item.role}</span> · <span data-g7pb-inline-field={`items.${active}.company`}>{item.company}</span></small></footer></blockquote><nav aria-label="후기 선택">{items.map((entry, index) => <button type="button" aria-current={active === index} onClick={() => setActive(index)} key={`${entry.name}-${index}`}>{index + 1}</button>)}</nav></div></Frame>;
}
