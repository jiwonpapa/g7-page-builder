...
export async function insertSavedComposition(host: NativeHost, library: NativeCompositionLibrary, id: string, target: string, signal: AbortSignal): Promise<NativeMediaResult<null>> {
  ...
  const result = await host.compositions.insert(loaded.data.snapshot, target, slot.items.length, signal);
