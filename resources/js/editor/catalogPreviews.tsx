...
export function G7RecentPostsPreview(props: G7RecentPostsEditorProps & { id: string }): React.ReactElement {
  return <BlockFrame id={props.id} type="g7-recent-posts" motion={props.motion} elementStyles={props.elementStyles}><div className={`g7pb-preview-g7-data ${surfaceClass(props.surface, props.spacing, props.textScale, props.textAlign)}`}><header><small data-g7pb-inline-field="eyebrow">{props.eyebrow}</small><RichTextCanvasField as="h2" className="g7pb-preview-richtext" fieldPath="heading">{props.heading}</RichTextCanvasField><em>G7 게시판 · {props.source === 'board' ? `${props.boardSlug || '게시판을 선택해 주세요'} · 최신순` : props.source === 'recent' ? '전체 최신순' : '전체 인기순'} · {props.limit}개</em></header><div className="g7pb-preview-post-list">{['페이지 제작 소식을 전합니다', '새로운 기능 업데이트 안내', '자주 묻는 질문을 확인하세요'].map((title, index) => <article key={title}><span>{index + 1}</span><div><strong>{title}</strong><small>게시판 이름 · 방금 전</small></div><b>→</b></article>)}</div><p className="g7pb-preview-data-note">실제 공개 게시글은 미리보기·발행 화면에서 G7 공개 API로 불러옵니다.</p></div></BlockFrame>;
}
