...
task-restack-squash:
	@test -n "$(TASK)" || { echo 'TASK is required.' >&2; exit 2; }
	@test -n "$(NEW_BASE_REF)" || { echo 'NEW_BASE_REF is required.' >&2; exit 2; }
	@$(COORD_HARNESS) restack-squash --task "$(TASK)" --new-base-ref "$(NEW_BASE_REF)"

task-replace-submitted:
	@test -n "$(TASK)" || { echo 'TASK is required.' >&2; exit 2; }
