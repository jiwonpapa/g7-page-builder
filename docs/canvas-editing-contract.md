...
- 현재 블록 라이브러리는 16:10 썸네일, 제목·설명, 기본 요소·레이아웃·컴포넌트·완성 섹션의 제작 분류와 용도·출처 필터를 사용합니다. 선택 위치의 허용 후보와 삽입 불가 이유를 같은 구조 정책에서 구하며 프리셋 전체를 완성 섹션으로 부르지 않습니다. hover 확대 복제 화면은 만들지 않습니다. 기존 Split Hero는 렌더 호환으로 보존하고 신규 기본 후보에서 숨깁니다. 분할 프리셋은 Hero의 layout으로 삽입합니다.
...
- 글자 범위는 블록 ID와 필드 경로가 같은 동안만 유지합니다. 이전 필드의 늦은 범위 해제는 현재 필드의 선택을 해제하지 않습니다.
- 부모 복귀·대상 삭제·권한 상실 시 해당 요소 도구를 닫습니다. 이미지/연결 선택창은 열 당시 필드를 소유하고, 대상이 바뀌거나 닫힌 뒤 도착한 결과를 적용하지 않습니다. 같은 대상의 결과는 최신 Puck 데이터에 적용하여 다른 속성 수정을 보존합니다.
- 범위 이벤트는 실행 시 형식을 검증합니다. 이 UI 상태는 저장 문서 원본과 별개이며 편집 정책의 지원 필드를 늘리지 않습니다.

- Undo/Redo는 Puck의 기존 히스토리를 사용합니다. 250ms 안의 빠른 연속 명령은 한 묶음으로 합쳐질 수 있습니다. 한 클릭당 독립 기록을 보장하지 않으며, 묶음의 원본·결과·항목 스타일/순서가 복원되어야 합니다.
- 반복 항목 명령은 현재 항목의 배열 경로와 스타일 경로를 함께 갱신합니다. 블록 명령과 구별하고, 임의의 내부 DOM 삽입/이동 기능으로 해석하지 않습니다.
- 본문 블록의 태블릿·모바일 값은 각각 공통값에서 상속합니다. 태블릿에서 모바일로 연쇄 상속하지 않습니다. 스타일 초기화와 레이아웃 초기화는 해당 부분만 지우며 모바일 Columns는 1열 고정입니다. Header/Footer의 별도 반응형 계약과 혼동하지 않습니다.
