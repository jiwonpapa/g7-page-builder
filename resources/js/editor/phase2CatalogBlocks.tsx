...
const DEFAULT_TESTIMONIALS: TestimonialsEditorProps = {
  ...
  layout: 'grid', surface: 'soft', spacing: 'normal', motion: { ...DEFAULT_BLOCK_MOTION },
};
...
const DEFAULT_ARTICLES: ArticleListEditorProps = {
  ...
  layout: 'list', surface: 'default', spacing: 'normal', motion: { ...DEFAULT_BLOCK_MOTION },
};
...
export const phase2CatalogComponentConfigs: Config<Phase2CatalogEditorComponents>['components'] = {
  Testimonials: {
    ...
    fields: { eyebrow: { type: 'text', label: '보조 문구', contentEditable: true }, heading: { type: 'text', label: '제목', contentEditable: true }, layout: { type: 'radio', label: '배치', options: [{ label: '그리드', value: 'grid' }, { label: '집중형', value: 'spotlight' }] }, items: { type: 'array', label: '후기', min: 2, max: 8, defaultItemProps: (index) => ({ quote: '고객 경험을 입력하세요.', name: `고객 ${index + 1}`, role: '담당자', company: '회사명', avatarSrc: '', avatarAlt: '', rating: 5 }), getItemSummary: (item) => `${item.name} · ${item.company}`, arrayFields: { quote: createRichTextField('후기', 120), name: { type: 'text', label: '이름', contentEditable: true }, role: { type: 'text', label: '역할', contentEditable: true }, company: { type: 'text', label: '회사', contentEditable: true }, avatarSrc: createMediaField('프로필 이미지'), avatarAlt: { type: 'text', label: '대체 텍스트' }, rating: { type: 'select', label: '평점', options: [1, 2, 3, 4, 5].map((value) => ({ label: `${value}점`, value })) } } }, ...appearanceFields, motion: createMotionField(['none', 'reveal', 'stagger']) },
    render: (props) => <TestimonialsPreview {...props} />,
  },
  FaqAccordion: {
    label: 'FAQ 아코디언', defaultProps: DEFAULT_FAQ,
    fields: { eyebrow: { type: 'text', label: '보조 문구', contentEditable: true }, heading: { type: 'text', label: '제목', contentEditable: true }, behavior: { type: 'radio', label: '열림 방식', options: [{ label: '하나만', value: 'single' }, { label: '여러 개', value: 'multiple' }] }, openFirst: { type: 'radio', label: '첫 항목', options: [{ label: '열기', value: true }, { label: '닫기', value: false }] }, items: { type: 'array', label: '질문과 답변', min: 2, max: 12, defaultItemProps: (index) => ({ question: `질문 ${index + 1}`, answer: '답변을 입력하세요.' }), getItemSummary: (item) => item.question, arrayFields: { question: { type: 'text', label: '질문', contentEditable: true }, answer: createRichTextField('답변', 140) } }, ...appearanceFields, motion: createMotionField(['none', 'reveal']) },
    render: (props) => <FaqPreview {...props} />,
  },
  ProcessTimeline: {
    label: '프로세스·타임라인', defaultProps: DEFAULT_PROCESS,
    fields: { eyebrow: { type: 'text', label: '보조 문구', contentEditable: true }, heading: { type: 'text', label: '제목', contentEditable: true }, layout: { type: 'radio', label: '배치', options: [{ label: '가로', value: 'horizontal' }, { label: '세로', value: 'vertical' }] }, items: { type: 'array', label: '단계', min: 2, max: 8, defaultItemProps: (index) => ({ title: `${index + 1}단계`, body: '단계 설명을 입력하세요.', linkLabel: '', linkUrl: '' }), getItemSummary: (item) => item.title, arrayFields: { title: { type: 'text', label: '제목', contentEditable: true }, body: { type: 'textarea', label: '설명', contentEditable: true }, linkLabel: { type: 'text', label: '링크 문구', contentEditable: true }, linkUrl: createRouteUrlField('링크 연결') } }, ...appearanceFields, motion: createMotionField(['none', 'reveal', 'stagger']) },
    render: (props) => <ProcessPreview {...props} />,
  },
  Tabs: {
    label: '탭 콘텐츠', defaultProps: DEFAULT_TABS,
    fields: { eyebrow: { type: 'text', label: '보조 문구', contentEditable: true }, heading: { type: 'text', label: '제목', contentEditable: true }, tabVariant: { type: 'radio', label: '탭 모양', options: [{ label: '밑줄', value: 'underline' }, { label: '버튼', value: 'pills' }] }, initialTab: { type: 'select', label: '처음 보일 탭', options: [0, 1, 2, 3, 4, 5].map((value) => ({ label: `${value + 1}번 탭`, value: String(value) })) }, items: { type: 'array', label: '탭', min: 2, max: 6, defaultItemProps: (index) => ({ label: `탭 ${index + 1}`, heading: '탭 제목', body: '탭 내용을 입력하세요.' }), getItemSummary: (item) => item.label, arrayFields: { label: { type: 'text', label: '탭 이름', contentEditable: true }, heading: { type: 'text', label: '내용 제목', contentEditable: true }, body: { type: 'textarea', label: '내용', contentEditable: true } } }, ...appearanceFields, motion: createMotionField(['none', 'reveal']) },
    render: (props) => <TabsPreview {...props} />,
  },
  ComparisonTable: {
    label: '비교표', defaultProps: DEFAULT_COMPARISON,
    fields: { eyebrow: { type: 'text', label: '보조 문구', contentEditable: true }, heading: { type: 'text', label: '제목', contentEditable: true }, highlightColumn: { type: 'select', label: '강조 열', options: [{ label: '강조 없음', value: 'none' }, ...[0, 1, 2, 3].map((value) => ({ label: `${value + 1}번 열`, value: String(value) }))] }, columns: { type: 'array', label: '비교 대상', min: 2, max: 4, defaultItemProps: (index) => ({ title: `선택 ${index + 1}`, description: '간단한 설명' }), getItemSummary: (item) => item.title, arrayFields: { title: { type: 'text', label: '이름', contentEditable: true }, description: { type: 'textarea', label: '설명', contentEditable: true } } }, rows: { type: 'array', label: '비교 항목', min: 1, max: 12, defaultItemProps: (index) => ({ feature: `항목 ${index + 1}`, valuesText: '지원\n지원' }), getItemSummary: (item) => item.feature, arrayFields: { feature: { type: 'text', label: '항목', contentEditable: true }, valuesText: { type: 'textarea', label: '열별 값(한 줄에 하나)' } } }, ...appearanceFields, motion: createMotionField(['none', 'reveal']) },
    render: (props) => <ComparisonPreview {...props} />,
  },
  ArticleList: {
    label: '에디토리얼 목록', defaultProps: DEFAULT_ARTICLES,
    fields: { eyebrow: { type: 'text', label: '보조 문구', contentEditable: true }, heading: { type: 'text', label: '제목', contentEditable: true }, layout: { type: 'radio', label: '배치', options: [{ label: '목록', value: 'list' }, { label: '그리드', value: 'grid' }] }, items: { type: 'array', label: '콘텐츠', min: 2, max: 8, defaultItemProps: (index) => ({ category: '소식', title: `콘텐츠 ${index + 1}`, summary: '콘텐츠 설명을 입력하세요.', date: '', imageSrc: '', imageAlt: '', url: '/' }), getItemSummary: (item) => item.title, arrayFields: { category: { type: 'text', label: '분류', contentEditable: true }, title: { type: 'text', label: '제목', contentEditable: true }, summary: { type: 'textarea', label: '요약', contentEditable: true }, date: { type: 'text', label: '날짜', contentEditable: true }, imageSrc: createMediaField('대표 이미지'), imageAlt: { type: 'text', label: '대체 텍스트' }, url: createRouteUrlField('콘텐츠 연결') } }, ...appearanceFields, motion: createMotionField(['none', 'reveal', 'stagger']) },
    render: (props) => <ArticleListPreview {...props} />,
  },
  VideoEmbed: {
    label: '영상', defaultProps: DEFAULT_VIDEO,
    fields: { eyebrow: { type: 'text', label: '보조 문구', contentEditable: true }, heading: { type: 'text', label: '제목', contentEditable: true }, caption: { type: 'textarea', label: '영상 설명', contentEditable: true }, provider: { type: 'radio', label: '영상 서비스', options: [{ label: 'YouTube', value: 'youtube' }, { label: 'Vimeo', value: 'vimeo' }] }, videoId: { type: 'text', label: '영상 ID' }, ratio: { type: 'select', label: '화면 비율', options: [{ label: '16:9', value: '16:9' }, { label: '4:3', value: '4:3' }, { label: '1:1', value: '1:1' }] }, ...appearanceFields, motion: createMotionField(['none', 'reveal']) },
...
export function canonicalPhase2BlockToPuck(block: PageBuilderBlock): { type: Phase2ComponentType; props: Phase2CatalogEditorComponents[Phase2ComponentType] } | null {
  ...
  if (block.type === TESTIMONIALS_BLOCK_TYPE) return { type: 'Testimonials', props: { eyebrow: asString(props.eyebrow), heading: asString(props.heading), items: normalizeTestimonials(props.items), layout: props.layout === 'spotlight' ? 'spotlight' : 'grid', ...common({ surface: 'soft', spacing: 'normal' }) } };
  ...
  if (block.type === ARTICLE_LIST_BLOCK_TYPE) return { type: 'ArticleList', props: { eyebrow: asString(props.eyebrow), heading: asString(props.heading), items: normalizeArticles(props.items), layout: props.layout === 'grid' ? 'grid' : 'list', ...common({ surface: 'default', spacing: 'normal' }) } };
...
export function phase2PuckBlockToCanonical(type: string, raw: Record<string, unknown>, includeAppearance: boolean): { type: string; props: Record<string, unknown> } | null {
  if (type === 'Testimonials') return { type: TESTIMONIALS_BLOCK_TYPE, props: attachAppearance({ eyebrow: asString(raw.eyebrow), heading: asString(raw.heading), items: normalizeTestimonials(raw.items), layout: raw.layout === 'spotlight' ? 'spotlight' : 'grid' }, raw, { surface: 'soft', spacing: 'normal' }, includeAppearance) };
  ...
  if (type === 'ArticleList') return { type: ARTICLE_LIST_BLOCK_TYPE, props: attachAppearance({ eyebrow: asString(raw.eyebrow), heading: asString(raw.heading), items: normalizeArticles(raw.items), layout: raw.layout === 'grid' ? 'grid' : 'list' }, raw, { surface: 'default', spacing: 'normal' }, includeAppearance) };
