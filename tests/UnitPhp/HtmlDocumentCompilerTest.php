...
final class HtmlDocumentCompilerTest extends TestCase
{
    ...
    public function test_responsibility_extraction_preserves_existing_catalog_artifact_bytes(): void
    {
        ...
        $documents = [
            ...
            [PageBuilderDocument::fromArray($this->catalogPayload()), 'dd8dccc0fe97eb24169d8240d58304ced5b483d8bd5386796c3aee1f919ab912', ['Hero 계열 블록이 2개 있습니다. 첫 화면 집중도가 낮아질 수 있습니다.']],
            ...
            [$layout, 'ae11827441cd95c105fe00de5d9a2afb3f3a8620a0b5331e7b197604bc226a8a', []],
        ];
        $compiler = $this->builtInCompiler();
        foreach ($documents as $index => [$document, $expectedSha, $expectedWarnings]) {
            $revision = 17 + $index;
            $result = $compiler->compile($document, $revision, 'html', 'g7-7.0.7');

            self::assertIsString($result->artifact, $document->slug);
            self::assertSame($expectedSha, hash('sha256', $result->artifact), $document->slug);
            self::assertSame($expectedSha, $result->artifactSha256, $document->slug);
            self::assertSame([
                'g7-page-builder-compile-result/v1', '0.19.0', $document->documentId,
                $revision, 'html', 'g7-7.0.7', $expectedWarnings,
            ], [
                $result->schemaVersion, $result->compilerVersion, $result->documentId,
                $result->sourceRevision, $result->targetFormat, $result->targetEngineVersion, $result->warnings,
            ], $document->slug);
        }
    }
    ...
    public function test_compiles_three_columns_and_a_nested_stack_without_losing_leaf_order(): void
    {
        $contents = file_get_contents(dirname(__DIR__).'/Contract/document-layout-v2.fixture.json');
        self::assertIsString($contents);
        $payload = json_decode($contents, true, flags: JSON_THROW_ON_ERROR);
        $columns = &$payload['blocks'][0]['slots']['content'][0];
        ...
        $columns['slots']['column3'] = [[
            ...
            'slots' => ['content' => [[
                ...
                'block_version' => 1,
                'props' => ['variant' => 'solid', 'width' => 'standard', 'label' => '세 번째 열'],
            ]]],
        ]];
        unset($columns);
    ...
    public function test_all_builtin_presets_compile_as_typed_documents(): void
    {
        ...
        self::assertCount($quality['contract']['inventory']['preset_count'], $manifest['presets']);
