...
Hero·ImageText는 구조 편집 활성화 후 명시적으로 버튼 구역으로 전환한다. 기존 block_version=1 속성 형식을 유지하는 선택형 v2 슬롯 확장이며, 기존 primaryCta/primaryLink는 `slots.actions`가 없는 문서에만 사용한다. actions가 빈 배열이어도 이전 버튼으로 되돌아가지 않는다. 두 소유자를 동시에 저장하는 것은 거부한다. 전환은 문구·URL·지원된 문구 스타일을 같은 Puck Undo 항목으로 옮긴다. 원본에 별도 편집기 소유권 플래그를 저장하지 않는다.
