...
export async function validateEditorLayoutParity(root) {
  ...
  const cssContract = [
    ...
    [/--g7pb-page-content-max:\s*80rem;[\s\S]*\.g7pb-full-site-page--template\s*\{[^}]*max-width:\s*var\(--g7pb-page-content-max\);[^}]*padding-inline:\s*1rem;/,
      '편집기 template shell은 공통 1280px 콘텐츠 폭과 모바일 여백을 사용해야 합니다.'],
