...
export const phase2CatalogComponentConfigs: Config<Phase2CatalogEditorComponents>['components'] = {
  Testimonials: {
    ...
    fields: { eyebrow: { type: 'text', label: '보조 문구', contentEditable: true }, heading: { type: 'text', label: '제목', contentEditable: true }, layout: { type: 'select', label: '레이아웃', options: [{ label: '후기 월', value: 'wall' }, { label: '균등 그리드', value: 'grid' }, { label: '한 줄 집중', value: 'spotlight' }, { label: '제목 분할', value: 'split' }, { label: '대형 인용', value: 'quote-hero' }] }, items: { type: 'array', label: '후기', min: 2, max: 8, defaultItemProps: (index) => ({ quote: '고객 경험을 입력하세요.', name: `고객 ${index + 1}`, role: '담당자', company: '회사명', avatarSrc: '', avatarAlt: '', rating: 5 }), getItemSummary: (item) => `${item.name} · ${item.company}`, arrayFields: { quote: createRichTextField('후기', 120), name: { type: 'text', label: '이름', contentEditable: true }, role: { type: 'text', label: '역할', contentEditable: true }, company: { type: 'text', label: '회사', contentEditable: true }, avatarSrc: createMediaField('프로필 이미지'), avatarAlt: { type: 'text', label: '대체 텍스트' }, rating: { type: 'select', label: '평점', options: [1, 2, 3, 4, 5].map((value) => ({ label: `${value}점`, value })) } } }, ...appearanceFields, motion: createMotionField(['none', 'reveal', 'stagger']) },
