...
const DEFAULT_BOARD_ARCHIVE: G7BoardArchiveEditorProps = { eyebrow: '콘텐츠 아카이브', heading: '관심 있는 이야기를 찾아보세요', source: 'recent', period: 'month', limit: '12', audience: 'all', showSearch: true, showBoardFilter: true, emptyMessage: '조건에 맞는 게시글이 없습니다.', surface: 'default', spacing: 'normal', motion: { ...DEFAULT_BLOCK_MOTION } };
const DEFAULT_PRODUCT_SHOWCASE: G7ProductShowcaseEditorProps = { eyebrow: '추천 상품', heading: '지금 주목할 상품', source: 'new', limit: '6', audience: 'all', detailBasePath: '/shop/products', layout: 'featured', emptyMessage: '표시할 상품이 없습니다.', surface: 'soft', spacing: 'normal', motion: { ...DEFAULT_BLOCK_MOTION } };
...
function appearance(value: unknown, fallback: BlockAppearance): BlockAppearance { const record = asRecord(value); return { surface: record.surface === 'soft' || record.surface === 'contrast' ? record.surface : fallback.surface, spacing: record.spacing === 'compact' || record.spacing === 'spacious' ? record.spacing : fallback.spacing, ...(record.textScale === 'compact' || record.textScale === 'large' ? { textScale: record.textScale } : {}), ...(record.textAlign === 'center' || record.textAlign === 'right' ? { textAlign: record.textAlign } : {}) }; }
function attachAppearance(props: Record<string, unknown>, raw: Record<string, unknown>, fallback: BlockAppearance, include: boolean): Record<string, unknown> { const resolved = appearance({ surface: raw.surface, spacing: raw.spacing, textScale: raw.textScale, textAlign: raw.textAlign }, fallback); return include || resolved.surface !== fallback.surface || resolved.spacing !== fallback.spacing || resolved.textScale || resolved.textAlign ? { ...props, appearance: resolved } : props; }
...
function Frame({ id, type, motion, children }: { id: string; type: string; motion: BlockMotion; children: React.ReactNode }): React.ReactElement { return <section className="g7pb-preview-block" data-testid="page-builder-block" data-block-id={id} data-block-type={type} {...motionPreviewAttributes(motion)}>{children}</section>; }

function LogoCarouselPreview(props: LogoCarouselEditorProps & { id: string }): React.ReactElement {
  return <Frame id={props.id} type="logo-carousel" motion={props.motion}><div className={`g7pb-preview-logo-carousel ${surfaceClass(props)}`}><header><small data-g7pb-inline-field="eyebrow">{props.eyebrow}</small><h2 data-g7pb-inline-field="heading">{props.heading}</h2></header><div>{normalizeLogos(props.logos).map((logo, index) => <span key={`${logo.name}-${index}`}>{logo.imageSrc ? <img src={logo.imageSrc} alt={logo.imageAlt} /> : <b data-g7pb-inline-field={`logos.${index}.name`}>{logo.name}</b>}</span>)}</div><footer><button type="button" aria-label="이전 로고">←</button><i>자동 {props.autoplay === 'yes' ? '재생' : '정지'}</i><button type="button" aria-label="다음 로고">→</button></footer></div></Frame>;
}
...
function TestimonialSliderPreview(props: TestimonialSliderEditorProps & { id: string }): React.ReactElement {
  ...
  return <Frame id={props.id} type="testimonial-slider" motion={props.motion}><div className={`g7pb-preview-testimonial-slider ${surfaceClass(props)}`}><header><small data-g7pb-inline-field="eyebrow">{props.eyebrow}</small><h2 data-g7pb-inline-field="heading">{props.heading}</h2></header><blockquote><span>{'★'.repeat(item.rating)}</span><p data-g7pb-inline-field={`items.${active}.quote`}>“{item.quote}”</p><footer><strong data-g7pb-inline-field={`items.${active}.name`}>{item.name}</strong><small><span data-g7pb-inline-field={`items.${active}.role`}>{item.role}</span> · <span data-g7pb-inline-field={`items.${active}.company`}>{item.company}</span></small></footer></blockquote><nav aria-label="후기 선택">{items.map((entry, index) => <button type="button" aria-current={active === index} onClick={() => setActive(index)} key={`${entry.name}-${index}`}>{index + 1}</button>)}</nav></div></Frame>;
}

function EventSchedulePreview(props: EventScheduleEditorProps & { id: string }): React.ReactElement {
  return <Frame id={props.id} type="event-schedule" motion={props.motion}><div className={`g7pb-preview-events g7pb-preview-events--${props.layout} ${surfaceClass(props)}`}><header><small data-g7pb-inline-field="eyebrow">{props.eyebrow}</small><h2 data-g7pb-inline-field="heading">{props.heading}</h2></header><ol>{normalizeEvents(props.items).map((item, index) => <li key={`${item.date}-${item.title}-${index}`}><time><b data-g7pb-inline-field={`items.${index}.date`}>{item.date}</b><span data-g7pb-inline-field={`items.${index}.time`}>{item.time}</span></time><div><small data-g7pb-inline-field={`items.${index}.location`}>{item.location}</small><h3 data-g7pb-inline-field={`items.${index}.title`}>{item.title}</h3><p data-g7pb-inline-field={`items.${index}.description`}>{item.description}</p><b data-g7pb-inline-field={`items.${index}.buttonLabel`}>{item.buttonLabel} →</b></div></li>)}</ol></div></Frame>;
}

function DownloadResourcesPreview(props: DownloadResourcesEditorProps & { id: string }): React.ReactElement {
  return <Frame id={props.id} type="download-resources" motion={props.motion}><div className={`g7pb-preview-downloads ${surfaceClass(props)}`}><header><small data-g7pb-inline-field="eyebrow">{props.eyebrow}</small><h2 data-g7pb-inline-field="heading">{props.heading}</h2></header><ul>{normalizeDownloads(props.items).map((item, index) => <li key={`${item.title}-${index}`}><span>{item.fileType || 'FILE'}</span><div><h3 data-g7pb-inline-field={`items.${index}.title`}>{item.title}</h3><p data-g7pb-inline-field={`items.${index}.description`}>{item.description}</p><small><span data-g7pb-inline-field={`items.${index}.fileType`}>{item.fileType}</span> · <span data-g7pb-inline-field={`items.${index}.fileSize`}>{item.fileSize}</span></small></div><b data-g7pb-inline-field={`items.${index}.buttonLabel`}>{item.buttonLabel} ↓</b></li>)}</ul></div></Frame>;
}
...
export function canonicalPhase3BlockToPuck(block: PageBuilderBlock): { type: Phase3ComponentType; props: Phase3CatalogEditorComponents[Phase3ComponentType] } | null {
  ...
  if (block.type === G7_BOARD_ARCHIVE_BLOCK_TYPE) return { type: 'G7BoardArchive', props: { eyebrow: asString(props.eyebrow), heading: asString(props.heading), source: props.source === 'popular' ? 'popular' : 'recent', period: ['today', 'week', 'year'].includes(asString(props.period)) ? asString(props.period) as G7BoardArchiveEditorProps['period'] : 'month', limit: ['6', '8'].includes(String(props.limit)) ? String(props.limit) as G7BoardArchiveEditorProps['limit'] : '12', audience: props.audience === 'guest' || props.audience === 'member' ? props.audience : 'all', showSearch: props.showSearch !== false, showBoardFilter: props.showBoardFilter !== false, emptyMessage: asString(props.emptyMessage, DEFAULT_BOARD_ARCHIVE.emptyMessage), ...common(block, { surface: 'default', spacing: 'normal' }) } };
  if (block.type === G7_PRODUCT_SHOWCASE_BLOCK_TYPE) return { type: 'G7ProductShowcase', props: { eyebrow: asString(props.eyebrow), heading: asString(props.heading), source: props.source === 'popular' || props.source === 'latest' ? props.source : 'new', limit: ['3', '4', '8'].includes(String(props.limit)) ? String(props.limit) as G7ProductShowcaseEditorProps['limit'] : '6', audience: props.audience === 'guest' || props.audience === 'member' ? props.audience : 'all', detailBasePath: asString(props.detailBasePath, '/shop/products'), layout: props.layout === 'rail' ? 'rail' : 'featured', emptyMessage: asString(props.emptyMessage, DEFAULT_PRODUCT_SHOWCASE.emptyMessage), ...common(block, { surface: 'soft', spacing: 'normal' }) } };
  return null;
}
