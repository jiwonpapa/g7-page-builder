...
export const phase2CatalogComponentConfigs: Config<Phase2CatalogEditorComponents>['components'] = {
  ...
  ProcessTimeline: {
    ...
    fields: { eyebrow: { type: 'text', label: '보조 문구', contentEditable: true }, heading: { type: 'text', label: '제목', contentEditable: true }, layout: { type: 'radio', label: '배치', options: [{ label: '가로', value: 'horizontal' }, { label: '세로', value: 'vertical' }] }, items: { type: 'array', label: '단계', min: 2, max: 8, defaultItemProps: (index) => ({ title: `${index + 1}단계`, body: '단계 설명을 입력하세요.', linkLabel: '', linkUrl: '' }), getItemSummary: (item) => item.title, arrayFields: { title: { type: 'text', label: '제목', contentEditable: true }, body: createRichTextField('설명', 130), linkLabel: { type: 'text', label: '링크 문구', contentEditable: true }, linkUrl: createRouteUrlField('링크 연결') } }, ...appearanceFields, motion: createMotionField(['none', 'reveal', 'stagger']) },
    render: (props) => <ProcessPreview {...props} />,
  },
  Tabs: {
    label: '탭 콘텐츠', defaultProps: DEFAULT_TABS,
    fields: { eyebrow: { type: 'text', label: '보조 문구', contentEditable: true }, heading: { type: 'text', label: '제목', contentEditable: true }, tabVariant: { type: 'radio', label: '탭 모양', options: [{ label: '밑줄', value: 'underline' }, { label: '버튼', value: 'pills' }] }, initialTab: { type: 'select', label: '처음 보일 탭', options: [0, 1, 2, 3, 4, 5].map((value) => ({ label: `${value + 1}번 탭`, value: String(value) })) }, items: { type: 'array', label: '탭', min: 2, max: 6, defaultItemProps: (index) => ({ label: `탭 ${index + 1}`, heading: '탭 제목', body: '탭 내용을 입력하세요.' }), getItemSummary: (item) => item.label, arrayFields: { label: { type: 'text', label: '탭 이름', contentEditable: true }, heading: { type: 'text', label: '내용 제목', contentEditable: true }, body: createRichTextField('내용', 150) } }, ...appearanceFields, motion: createMotionField(['none', 'reveal']) },
