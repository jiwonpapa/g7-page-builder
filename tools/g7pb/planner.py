...
def build_plan(root: Path, paths: list[str], *, base="HEAD", phase="submission", full=False):
    ...
    gates = {}
    def add(name, argv, inputs, reason, requires=(), runtime=False):
        gates[name] = Gate(name, tuple(argv), tuple(sorted(set(inputs))), reason, runtime, (), tuple(requires))
    def python_test(path):
        ...
        add("python:" + path, ["python3", "-B", "-m", "unittest", "discover", "-s", "tests/Harness", "-p", Path(path).name],
            python_inputs(root, path), "Changed Python module or dependency")
    ...
    for path in plan.paths:
        ...
        if path.endswith(".py"):
            ...
            if file.suffix in {".ts", ".tsx"}:
                add("browser:" + path, ["npx", "playwright", "test", path, "--retries=0"], [path, "playwright.config.ts", "package-lock.json"], "Changed browser scenario", ("node", "php", "g7", "browser"), True)
            else:
    ...
    for test in ts_tests:
        add("unit:" + test, ["npx", "vitest", "run", test], [test, *ts_sources, "package-lock.json", "vitest.config.ts"], "Related unit behavior", ("node",))
    if ts_sources:
    ...
    if css:
        add("css", ["npx", "stylelint", *css], [*css, "stylelint.config.mjs", "package-lock.json"], "Changed CSS only", ("node",))
    for test in php_tests:
        ...
        add("php:" + test, argv, [test, *php_sources, "composer.lock", "phpunit.xml.dist"], "Related PHP behavior", ("php", "g7") if g7 else ("php",), g7)
    if php_sources:
    ...
    if content:
        try:
            ...
            for item in select_changes(root, base, content):
                ...
                add("content:" + kind + ":" + ",".join(ids), ["python3", "-B", "scripts/g7pb.py", "content", "check", "--kind", kind, "--ids", ",".join(ids)], [*content, *python_inputs(root, "tools/g7pb/content.py")], "Changed content and declared consumers", ("node", "php"))
