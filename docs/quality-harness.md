...
전체 `quality-gate`도 Local 통합 runner가 발급한 Site Part fixture capability를 브라우저 프로세스에 전달하고 종료·실패 시 원래 활성 세트를 복원합니다. 보호용 세션이 실행되지 않았거나 복원이 실패하면 통과로 기록하지 않습니다. 브라우저 PNG 기준 이미지를 바꾸면 파일명의 추측이 아니라 해당 `*.spec.ts-snapshots`의 실제 spec을 소유자로 연결하고 그 브라우저 검사를 실행합니다. 이미지는 검사 입력에 포함되며 기준 갱신만으로 제품 통과를 주장하지 않습니다.
...
`make editor-status`는 `docs/productization/editor-progress.json` 원장으로 작업 상태와 선행 조건을 충족한 다음 작업을 표시합니다. 기존 구현 기반은 별도 기록하며 새 개발 완료 수의 분모에 넣지 않습니다. `make editor-plan-check`는 고정 계획/정책/상태판 경로, 문서 표시문과 로컬 파일 링크, 단계·수용 기준 ID, 의존 순환, 완료 작업의 구현·통합 SHA 기록과 필수 증거 종류, 원장과 Markdown 상태판의 일치를 확인합니다. SHA는 40자리 형식과 필수 기록만 검사하며 Git 객체 존재·조상 관계는 판정하지 않습니다. 얕은 CI 이력이나 다른 checkout의 오래된 이력 부재를 새 제품 실패로 해석하지 않습니다.

두 명령은 파일·원장·coordination 상태를 쓰거나 제품 시험·배포를 실행하지 않습니다. `python3 -B -m tools.g7pb.editor_progress status --json`은 구조화 결과, `status --markdown`은 상태판 전문을 stdout으로 출력합니다. 상태판 초기 생성·갱신은 해당 파일을 소유한 task에서 출력 내용을 명시적으로 저장합니다. `status`는 원장을 검증하고 표시하며 상태판 일치는 `check`가 담당합니다.
...
`EDITOR_PLAN_OK`는 문서와 진척 기록의 기계적 정합성입니다. 필수 증거 파일과 SHA 기록이 있다는 사실로 시험 결과의 진실성·현재 HEAD의 제품 기능 완성·전체 자연어 문서의 모순 부재를 보증하지 않습니다. `current` 문서의 로컬 링크를 검사하되 `historical`/`reference` 문서는 파일과 필수 표시문만 검사합니다. 과거 실행 당시의 로그 링크를 현재 완료 증거로 취급하거나 존재 여부 때문에 역사 문서를 고쳐 쓰지 않습니다. 별도로 선언한 완료 증거 파일은 항상 확인합니다. 외부 URL 접속과 Markdown anchor 판정은 수행하지 않습니다. 실제 편집 기능의 완료는 해당 수용 시나리오와 제품 검증 기록으로 별도 입증합니다.
