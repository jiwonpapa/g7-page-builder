...
현재 상태: **1~6차 코드 구현·관련 검증·동일 기준 재감사 완료, 0.32.0 초기 배포 뒤 종료 patch 통합, 0.32.1 재배포 최종 gate 대기**. 1차 제품 검증 SHA는 `38ec067ed6dbb68c3ac09ce4c8572f1ec6524edc`, 문서 통합 기준은 `e9a3afb1d18b87e50c5abffc00fc80294764c96d`다. 2차 제품 검증 SHA는 `d59bbc07482e878c179ab784a0e301c4c7c5a1e3`, 문서 통합·정식 종료 SHA는 `23beb94abdaba28038073c2116020440eb29eb79`다. 3차 제품 통합 SHA는 `cbe955a6c9f10cd131d7b7ce5ef588e1c14bed26`이며 Manager 1,597→96줄·catalog 851→68줄·공개 진입점 1,163→16줄로 책임을 분리했다. 정적 부채 1,066→1,061, canonical의 UI 금지 연결 44→0이며 신규 위반·순환·우회 단언 규칙 위반은 0이다. 각 차수의 문서 통합 후 최종 검증·NO_RELEASE 종료는 [1차](2026-09-02-structure-phase-1.md)·[2차](2026-09-02-structure-phase-2.md)·[3차 마감 기록](2026-09-03-structure-phase-3.md)과 연결된 영수증으로 확인한다. 4차 제품 통합 SHA는 `ae25c0dec6c8e714fff0ddd5b9c52ed5126ebf37`이며 PHP 본체 2,504→297줄·기본 renderer 45종 분리·전체 정적 부채 1,061→1,060을 확인했다. 문서 통합·정식 종료는 [4차 마감 기록](2026-09-03-structure-phase-4.md)의 최종 영수증과 연결한다. 5차 제품·감사 기준은 `3a747c68f5f6e613f9f1e4a117c4af505aa3621a`이며 스타일 부채 1,060→220와 CSS 크기 예외 0을 확인했다. [5차 마감 기록](2026-09-03-structure-phase-5.md)에 정식 종료 영수증을 연결한다. 6차 제품 통합 SHA는 `69f0ede5645b0b3c0acc028e6484ed40137f1588`이며 366파일·신규 오류 0·잔여 부채 220·unused debt 0과 공개 JS 세 예산 통과를 확인했다. 0.32.0 초기 배포 뒤 최종 종료 gate에서 세 운영 wrapper의 저장소 로컬 Python bytecode cache 생성을 적발했고, patch 제출 `8845de27f8a333b4f5b756768d175a6f8c17445d`를 `ad4aca04286ec956ee4fa3fd5d4524f490e36fc2`에 통합했다. [6차 기록](2026-09-04-structure-phase-6.md)은 코드 재감사와 0.32.1 patch 범위를 확정하며 0.32.1 package·push·재배포·smoke는 외부 영수증으로 별도 판정한다.
...
| 6. 재감사·0.32.1 종료 patch | 실제 감소량과 잔여 예외, import 방향·순환·타입/스타일 경계, 공개 JS 예산과 하네스 선택 범위 재검토 | 동일 기준으로 전후 비교, 새 위반 0, 해소된 예외 제거, 관련 행동 검사와 릴리스 범위 기록. 초기 배포 뒤 종료 gate에서 발견한 결함과 후속 patch를 분리 기록 | 정적 결과·소스 지문, 변경 관련 브라우저, 세 JS 예산, 0.32.0 초기 배포 뒤 wrapper red와 0.32.1 green·version gate. 재배포·smoke는 별도 외부 영수증 |
...
### 6차 — 코드 재감사·0.32.1 종료 patch

6차 코드 통합·관련 검증·동일 기준 재감사를 완료했다. 최종 제품 SHA는 `69f0ede5645b0b3c0acc028e6484ed40137f1588`이며 [6차 기록](2026-09-04-structure-phase-6.md)에 수치와 판정 경계를 기록한다. 0.32.0 초기 배포 뒤 적발한 종료 patch는 `ad4aca04286ec956ee4fa3fd5d4524f490e36fc2`에 통합했다.
...
- 0.32.0 초기 배포 뒤 최종 종료 gate의 합성 fixture에서 release·deploy·smoke wrapper 세 개가 모두 저장소 로컬 `tools/g7pb/__pycache__`를 생성했다. 원본 실패는 `structure-remediation/phase6-close/release-pycache-red.log`에 보존했다.
- 세 wrapper를 `python3 -B` 실행으로 고치고 기존 인자·환경·종료 상태 전달을 유지했다. release 하네스 19건과 세 shell syntax가 통과했으며 green은 `release-pycache-green.log`에 보존했다. 이는 wrapper 계약의 로컬 검증이며 0.32.1 재배포 성공 증거가 아니다.
- 0.32.1 버전·변경 기록은 wrapper patch 한 건만 release 대상으로 묶는다. 문서 task 뒤 최종 integration gate, package checksum, Git push, 원격 apply와 post-deploy smoke가 별도로 성공해야 0.32.1 릴리스·재배포 완료다.
...
| 0.32.0 릴리스·초기 배포 | 종료 gate 실패 | 초기 배포 뒤 세 운영 wrapper의 저장소 로컬 Python bytecode cache 생성 red를 적발해 최종 완료로 닫지 않음 |
| 0.32.1 종료 patch | 코드 통합 완료·재배포 대기 | 제출 `8845de27`, 통합 `ad4aca04`; release 19건·shell syntax와 cache 미생성 green. 0.32.1 package·push·원격 apply·smoke는 후속 외부 영수증으로 판정 |

[1차 구현·실패 보완·검증 기록](2026-09-02-structure-phase-1.md)에 제출/통합 SHA, 실제 검사, 부채 전후 수치를 기록한다. 6차 코드 재감사와 0.32.1 patch 범위 확정은 0.32.1 릴리스 패키지·push·운영 재배포 성공을 대신하지 않는다.
