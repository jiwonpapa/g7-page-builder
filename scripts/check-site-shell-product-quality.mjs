...
function validateMobileReview(review, expectedFingerprint) {
  ...
  for (const platform of ['ios-safari-voiceover', 'android-chrome-talkback']) {
...
if (process.argv.includes('--run')) {
  ...
  writeFileSync(evidencePath, JSON.stringify({ status: 'passed', fingerprint: before, checkedAt: new Date().toISOString(), viewports: ['desktop', 'tablet', 'mobile'], contractPersonas: ['guest', 'member', 'admin', 'unavailable'], mobileWidths: [320, 360, 390, 430, 768, 899, 900], browserEngines: ['chromium', 'webkit'], physicalDeviceApproval: mobileReviewPath, realG7: ['administrator', 'admin-route', 'native-logout', 'guest', 'standalone-builder', 'api-logout', 'editor-persona', 'editor-mobile-menu'] }, null, 2));
}
...
console.log(`Site Shell automated gate: PASS · 3 viewports · Chromium/WebKit · real G7 authentication (${evidence.checkedAt}). Release additionally requires current physical-device review.`);
