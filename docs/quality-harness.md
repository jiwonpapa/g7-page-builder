...
- frontend `task-submit`은 타입·단위시험 전에 `check:editor-acceptance`를 실행합니다. 전용 E2E가 실제 `mouse.down/move/up`, browser project와 360/768/1280 내부 canvas 일치, 활성 canvas iframe 고정, iframe border box의 transform 배율·content 원점을 반영한 좌표, 재시도 0회, 세 viewport, 툴바 상호배타, 저장·미리보기·공개 DOM 서식 증거를 잃거나 합성 Selection으로 바뀌면 제출을 거부합니다.
